...
final class WebSocketServer
{
    ...
    private function message(Worker $worker,TcpConnection $from,string $msg): void
    {
        ...
        try {
            ...
            if($kind==='command'){$result=$this->game->command($user,(string)($m['type']??''),(array)($m['payload']??[]),(string)($m['requestId']??''));$this->send($from,['type'=>'command.accepted','requestId'=>$m['requestId'],'event'=>$result]);if(in_array($result['type'],['scenario.activate','scenario.deactivate','scenario.hide'],true)){$this->broadcastScenarioListChanged($worker,(int)$result['scenarioId']);$this->broadcastGuestViews($worker);}$this->broadcastRefresh($worker,(int)$result['scenarioId']);return;}
            if($kind==='chat.send'){$scenarioId=(int)($m['scenarioId']??0);$msg=$this->game->sendChatMessage($scenarioId,$user,(array)($m['payload']??[]));$this->broadcastChat($worker,$scenarioId,$msg);return;}
            if($kind==='draw'){$scenarioId=(int)($m['scenarioId']??0);$payload=(array)($m['payload']??[]);$payload['userId']=(int)$user['id'];$payload['userName']=(string)$user['name'];$payload['role']=(string)$user['role'];if(($payload['op']??'')==='clearAll'&&$user['role']!=='DM')throw new RuntimeException('Solo el DM puede borrar todos los dibujos.');$this->broadcastDraw($worker,$scenarioId,$payload,$from);return;}
    ...
    private function broadcastChat(Worker $worker,int $sid,array $msg): void {foreach($worker->connections as $c){$meta=$this->meta[$c->id]??null;if(!$meta||($meta['scenario']??null)!==$sid)continue;$u=$meta['user'];if($u['role']==='DM'||($u['role']==='PLAYER'&&(int)$u['id']===(int)($msg['player_id']??0)))$this->send($c,['type'=>'chat.message','scenarioId'=>$sid,'data'=>$msg]);}}
    private function broadcastDraw(Worker $worker,int $sid,array $payload,TcpConnection $from): void {foreach($worker->connections as $c)if($c!==$from&&($this->meta[$c->id]['scenario']??null)===$sid)$this->send($c,['type'=>'draw.event','scenarioId'=>$sid,'data'=>$payload]);}
