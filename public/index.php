...
try {
    ...
    if(preg_match('#^/api/scenarios/(\d+)/chats$#',$path,$m)&&$method==='GET') jsonOut(['threads'=>$game->chatThreads((int)$m[1],$user)]);
    if(preg_match('#^/api/chats/(\d+)/messages$#',$path,$m)&&$method==='GET') jsonOut($game->chatMessages((int)$m[1],$user));
...
function ensureCsrf(): void { if(empty($_COOKIE['ttrpg_csrf']))setcookie('ttrpg_csrf',bin2hex(random_bytes(24)),['expires'=>time()+315360000,'path'=>'/','secure'=>(!empty($_SERVER['HTTPS'])&&$_SERVER['HTTPS']!=='off'),'httponly'=>false,'samesite'=>'Lax']); }
function verifyCsrf(): void { $a=$_COOKIE['ttrpg_csrf']??'';$b=$_SERVER['HTTP_X_CSRF_TOKEN']??'';if(!$a||!$b||!hash_equals($a,$b))throw new HttpError('Token CSRF inválido.',419); }
...
function codexActionCount(PDO $db,array $user,string $kind='actions'): int { $where=$user['role']==='DM'?'a.is_active=1':"a.is_active=1 AND vl.code='public' AND ac.code<>'monster_ability'"; $where.=$kind==='spells'?" AND ac.code='spell'":" AND ac.code<>'spell'"; return (int)$db->query("SELECT COUNT(*) FROM actions a JOIN action_categories ac ON ac.id=a.action_category_id JOIN visibility_levels vl ON vl.id=a.visibility_level_id WHERE $where")->fetchColumn(); }
function codexCategories(PDO $db,array $user): array {
    $categories=[
        ['code'=>'spells','name'=>'Conjuros','description'=>'Hechizos y magia reutilizable.','count'=>codexActionCount($db,$user,'spells')],
        ['code'=>'actions','name'=>'Acciones y rasgos','description'=>'Rasgos, acciones y habilidades reutilizables que no son conjuros.','count'=>codexActionCount($db,$user,'actions')],
...
function codexCategoryMeta(string $category): ?array {
    return match($category){
        'creatures'=>['table'=>'creatures','owner'=>'creature','select'=>'c.id,c.name,c.short_description,c.armor_class_text,c.hit_points_text,ct.name type_name,cs.name subtype_name,src.name source_name','joins'=>'LEFT JOIN creature_types ct ON ct.id=c.creature_type_id LEFT JOIN creature_sizes cs ON cs.id=c.creature_size_id LEFT JOIN sources src ON src.id=c.source_material_id','search'=>['c.name','c.short_description','c.description','ct.name','cs.name','src.name','c.challenge_rating_text','c.environment_text','c.traits_text','c.skills_text','c.saving_throws_text','c.senses_text','c.languages_text','c.damage_resistances_text','c.damage_immunities_text','c.damage_vulnerabilities_text','c.condition_immunities_text','c.equipment_text']],
        'items'=>['table'=>'items','owner'=>'item','select'=>'c.id,c.name,c.short_description,it.name type_name,ir.name subtype_name,src.name source_name','joins'=>'LEFT JOIN item_types it ON it.id=c.item_type_id LEFT JOIN item_rarities ir ON ir.id=c.item_rarity_id LEFT JOIN sources src ON src.id=c.source_material_id','search'=>['c.name'],'nameOnly'=>true],
        'feats'=>['table'=>'feats','owner'=>'feat','select'=>'c.id,c.name,c.short_description,ft.name type_name,src.name source_name','joins'=>'LEFT JOIN feat_types ft ON ft.id=c.feat_type_id LEFT JOIN sources src ON src.id=c.source_material_id','search'=>['c.name'],'nameOnly'=>true],
        'species'=>['table'=>'species','owner'=>'species','select'=>'c.id,c.name,c.short_description,c.lineage_type_code type_name,src.name source_name','joins'=>'LEFT JOIN sources src ON src.id=c.source_material_id','search'=>['c.name'],'nameOnly'=>true],
        'subspecies'=>['table'=>'subspecies','owner'=>'subspecies','select'=>'c.id,c.name,c.short_description,sp.name type_name,c.lineage_type_code subtype_name,src.name source_name','joins'=>'JOIN species sp ON sp.id=c.species_id LEFT JOIN sources src ON src.id=c.source_material_id','search'=>['c.name'],'nameOnly'=>true],
        'backgrounds'=>['table'=>'backgrounds','owner'=>'background','select'=>'c.id,c.name,c.short_description,c.background_type_code type_name,c.setting_name subtype_name,src.name source_name','joins'=>'LEFT JOIN sources src ON src.id=c.source_material_id','search'=>['c.name'],'nameOnly'=>true],
        'background_variants'=>['table'=>'background_variants','owner'=>'background_variant','select'=>'c.id,c.name,c.short_description,b.name type_name,c.background_type_code subtype_name,src.name source_name','joins'=>'JOIN backgrounds b ON b.id=c.background_id LEFT JOIN sources src ON src.id=c.source_material_id','search'=>['c.name'],'nameOnly'=>true],
        'classes'=>['table'=>'classes','owner'=>'class','select'=>'c.id,c.name,c.short_description,c.hit_die_text type_name,c.primary_ability_text subtype_name,src.name source_name','joins'=>'LEFT JOIN sources src ON src.id=c.source_material_id','search'=>['c.name'],'nameOnly'=>true],
        'subclasses'=>['table'=>'subclasses','owner'=>'subclass','select'=>'c.id,c.name,c.short_description,cl.name type_name,c.subclass_type_text subtype_name,src.name source_name','joins'=>'JOIN classes cl ON cl.id=c.class_id LEFT JOIN sources src ON src.id=c.source_material_id','search'=>['c.name'],'nameOnly'=>true],
        default=>null
    };
}
function codexCategoryRecords(PDO $db,array $user,string $category,string $q,int $page=1,int $limit=15): array {
    ...
    if($category==='actions'||$category==='spells'){
        $role=$user['role']; $visibility="JOIN visibility_levels vl ON vl.id=a.visibility_level_id"; $extra=$role==='DM'?'':" AND vl.code='public' AND ac.code<>'monster_ability'"; $extra.=$category==='spells'?" AND ac.code='spell'":" AND ac.code<>'spell'";
        if($q===''){
        ...
        $like='%'.$q.'%'; $where="a.is_active=1 $extra AND a.name LIKE ?"; $params=[$like];
    ...
    $like='%'.$q.'%'; $parts=array_map(fn($col)=>"$col LIKE ?",$meta['search']); $extraSearch=!empty($meta['nameOnly'])?'':" OR EXISTS(SELECT 1 FROM codex_record_tags crt JOIN tags t ON t.id=crt.tag_id WHERE crt.owner_type=? AND crt.owner_id=c.id AND (t.name LIKE ? OR t.code LIKE ?))";if($category==='creatures')$extraSearch.=" OR EXISTS(SELECT 1 FROM action_assignments aa JOIN actions a ON a.id=aa.action_id WHERE aa.owner_type='creature' AND aa.owner_id=c.id AND (a.name LIKE ? OR a.short_description LIKE ? OR a.description LIKE ?))"; $where="c.is_active=1 AND $visible AND (".implode(' OR ',$parts).$extraSearch.")"; $params=array_fill(0,count($meta['search']),$like);if(empty($meta['nameOnly']))$params=array_merge($params,[$meta['owner'],$like,$like]);if($category==='creatures')$params=array_merge($params,[$like,$like,$like]);
