...
try {
    ...
    $db->prepare('INSERT INTO player_characters(owner_id,campaign_id,name,max_health) VALUES (?,?,?,?)')->execute([$playerId,$campaign,'Compañero',16]);$secondCharacterId=(int)$db->lastInsertId();
    ...
    $placedSecond=$game->command($player,'player.place',['scenarioId'=>$scenarioId,'characterId'=>$secondCharacterId,'x'=>0,'y'=>2],req());
    $placedPlayers=$game->snapshot($scenarioId,$dm)['players'];ok(count(array_filter($placedPlayers,fn($p)=>(int)$p['user_id']===$playerId&&(bool)$p['placed']))===2,'jugador coloca más de un personaje en el mismo mapa');
    $firstPlaced=array_values(array_filter($placedPlayers,fn($p)=>(int)$p['character_id']===$characterId))[0];ok((int)$firstPlaced['image_asset_id']===$avatarId,'snapshot incluye el avatar del jugador');
    $move=$game->command($player,'movement.submit',['scenarioId'=>$scenarioId,'characterId'=>$characterId,'path'=>[['x'=>1,'y'=>0]]],req());ok($move['data']['status']==='APPLIED','movimiento libre se aplica al personaje seleccionado');
    $moveSecond=$game->command($player,'movement.submit',['scenarioId'=>$scenarioId,'characterId'=>$secondCharacterId,'path'=>[['x'=>1,'y'=>2]]],req());ok($moveSecond['data']['status']==='APPLIED','jugador controla independientemente su segundo personaje');
    $game->command($dm,'map.cells.paint',['scenarioId'=>$scenarioId,'cells'=>[['x'=>2,'y'=>0]],'blocked'=>true],req());
    $pending=$game->command($player,'movement.submit',['scenarioId'=>$scenarioId,'characterId'=>$characterId,'path'=>[['x'=>2,'y'=>0]]],req());ok($pending['data']['status']==='PENDING','movimiento bloqueado solicita aprobación');
    $approved=$game->command($dm,'movement.approve',['scenarioId'=>$scenarioId,'movementId'=>$pending['data']['id']],req());ok($approved['data']['status']==='APPLIED','el DM aprueba el movimiento del personaje correcto');
    $afterApproval=$game->snapshot($scenarioId,$dm)['players'];$firstAfter=array_values(array_filter($afterApproval,fn($p)=>(int)$p['character_id']===$characterId))[0];$secondAfter=array_values(array_filter($afterApproval,fn($p)=>(int)$p['character_id']===$secondCharacterId))[0];ok((int)$firstAfter['x']===2&&(int)$secondAfter['x']===1&&(int)$secondAfter['y']===2,'la aprobación no mueve los otros personajes del jugador');
    ...
    $snap=$game->snapshot($scenarioId,$dm);$spId=(int)array_values(array_filter($snap['players'],fn($p)=>(int)$p['character_id']===$characterId))[0]['id'];$secondSpId=(int)array_values(array_filter($snap['players'],fn($p)=>(int)$p['character_id']===$secondCharacterId))[0]['id'];
    ...
    $game->command($dm,'initiative.set',['scenarioId'=>$scenarioId,'kind'=>'PLAYER','id'=>$secondSpId,'initiative'=>5],req());
    ...
    try{$game->command($player,'movement.submit',['scenarioId'=>$scenarioId,'characterId'=>$secondCharacterId,'path'=>[['x'=>2,'y'=>2]]],req());ok(false,'segundo personaje no actúa fuera de su turno');}catch(RuntimeException){ok(true,'cada personaje respeta su propio turno de combate');}
    $combat=$game->snapshot($scenarioId,$dm);$npcParticipant=array_values(array_filter($combat['participants'],fn($p)=>$p['actor_type']==='NPC'))[0];$secondParticipant=array_values(array_filter($combat['participants'],fn($p)=>$p['actor_type']==='PLAYER'&&(int)$p['actor_id']===$secondSpId))[0];
    $wait=$game->command($player,'turn.delay',['scenarioId'=>$scenarioId,'targetParticipantId'=>(int)$npcParticipant['id']],req());ok($wait['data']['currentParticipantId']===(int)$secondParticipant['id'],'al retrasar, el turno continúa con el segundo personaje');
    $nextRound=$game->command($dm,'turn.next',['scenarioId'=>$scenarioId],req());ok($nextRound['data']['round']===2&&(int)$nextRound['data']['currentParticipantId']===(int)$npcParticipant['id'],'los personajes conservan su orden individual de iniciativa');
